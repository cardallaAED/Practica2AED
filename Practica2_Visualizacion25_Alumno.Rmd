...
```{r}
library(pacman)
packages =  c("tidyverse", "knitr", "ggplot2", "datasets", "RColorBrewer",'latex2exp','data.table')
...
grafico1
grafico2<-ggplot(data=ds,mapping= aes(x= MinTemp, y= MaxTemp, colour = RainTomorrow)) + geom_point()
grafico2
grafico3<-grafico2+ggtitle("Grafico 1")
grafico3
...
rm(list = ls())
load("./data/weatherAUS.RData")
ggplot(ds,aes(x= Temp9am, y= Pressure9am,shape=RainToday) )+geom_point()
ggplot(ds,aes(x= Temp9am, y= Pressure9am, colour = RainToday) )+geom_point() + labs(title="Probabilidad de lluvia segun condiciones meteorologicas",
                                                                                    subtitle ="Relacion entre la Lluvia y presinn" ,
                                                                                    x="Temperatura 9AM(ºC)",
                                                                                    y="Presion Atmosferica 9AM(mB)",
                                                                                    )
...
iris.grafico1<-ggplot(iris.tidy,aes(x=Species,y=Value,colour = Part))+geom_point()
iris.grafico2<-iris.grafico1+facet_grid(Measure~.)
iris.grafico2
iris.grafico2<-iris.grafico1+facet_grid(.~Measure)
iris.grafico2
ggplot(iris.tidy)+geom_jitter(mapping=aes(x=Species,y=Value,colour = Part))
...
ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour = Fabricante, group = Fabricante)) +geom_point()
ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour = Fabricante, group = Fabricante)) +geom_line(aes(linetype=Fabricante))+geom_point()
ggplot(data=DatosEjer4, aes(x=Fecha, y=Ventas, colour = Fabricante, group = Fabricante)) +geom_line(aes(linetype=Fabricante))+geom_point()+labs(title = "Ventas mensuales de diferentes compañías tecnológicas")
...
ggplot(data=Experimento, aes(x=Dosis, y=Altura, colour = Complemento)) +
  geom_line(aes(linetype=Complemento)) +
geom_point(aes(shape=Complemento))

ggplot(data=Experimento, aes(x=Dosis, y=Altura, colour = Complemento)) +
  geom_errorbar(aes(ymin = Altura - Error_Altura,
                    ymax = Altura + Error_Altura), width=0.1,position=position_dodge(0.05)) +
  geom_line(aes(linetype=Complemento)) +
  geom_point(aes(shape=Complemento))

ggplot(data=Experimento, aes(x=Dosis, y=Altura, colour = Complemento,shape = Complemento)) +
  geom_errorbar(aes(ymin = Altura - Error_Altura,
                    ymax = Altura + Error_Altura), width=0.1,position=position_dodge(0.05)) +
  geom_line() +
  geom_point()+
  labs(title = "Dependencia de altura con dosis",
       x="Altura(cm)",
       y="Dosis de Clorato")
...
ggplot(data=ice.river.df, aes(x = time)) +
  geom_line(aes(y = flow.vat, color = "red")) +
  geom_line(aes(y = flow.jok, color = "blue")) +labs(title = "Caudales de los rios islandeses Vatnsdalsa(azul) y Jokulsa Eystri(rojo)",x="Caudal(m3/s)",y="Tiempo(Años)" )
...
ggplot(data=ds, aes(x = Year, y=MinTemp)) +geom_boxplot()
...
ggplot(data=ds, aes(x = Year, y=MaxTemp, fill= Year)) +geom_violin()+labs(title="Distribución de temperaturas máximas por año",x=" ",y=" ")+
...
library(data.table)
library(ggplot2)
datos <- fread("http://assets.datacamp.com/blog_assets/chol.txt")

dataCamp <- as.data.frame(datos)
ggplot(data=dataCamp, aes(x=HEIGHT)) + geom_histogram()
ggplot(data=dataCamp, aes(x=HEIGHT)) +geom_histogram()+
labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia")

ggplot(data=dataCamp, aes(x=HEIGHT)) +geom_histogram(breaks=seq(155, 195),bins = 30)+
labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia")

ggplot(data=dataCamp, aes(x=HEIGHT)) +geom_histogram(breaks=seq(155, 195),fill = "blue", alpha = 0.5, color = "red", bins = 20)+
labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia")

ggplot(data=dataCamp, aes(x=HEIGHT)) +
  geom_histogram(aes(y =after_stat(density)),breaks=seq(155, 195),fill = "blue", alpha = 0.5, color = "red", bins = 20)+
